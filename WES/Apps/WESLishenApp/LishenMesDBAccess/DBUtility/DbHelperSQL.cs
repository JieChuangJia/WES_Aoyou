...
namespace LishenMesDBAccess.DBUtility
{
    /// <summary>
    /// 数据访问抽象基础类
    ...
    public abstract class DbHelperSQL
    {
        ...
        /// </summary>
        /// <param name="cmdText">存储路径</param>
        /// <param name="isBak">该操作是否为备份操作，是为true否，为false</param>
        /// <returns>返回值：1恢复成功，2备份成功，0失败</returns>
        public static int BakRecoverSql(string path, bool isBak)
        {
            ...
            try
            {
                ...
                string dbname = PubConstant.ConnectionString.Split(';')[1].Split('=')[1];
                if (!isBak) //如果是恢复操作
                {
                    cmdStr = "use master restore database " + dbname + " from disk='" + path + " '" + " with replace";
                    cmdBakRst.CommandText = cmdStr;
                }
                else//备份
                {
                  
                    cmdStr = @"backup database " + dbname + " to disk='" + path + "'";
                    cmdBakRst.CommandText = cmdStr;
                }
        ...
        public static void AsyncExecuteNonQuery(string sqlText, AsyncCallback callBack)
        {
            ...
            try
            {
                ...
                connection.ConnectionString = PubConstant.ConnectionString + "; Asynchronous Processing=true";
